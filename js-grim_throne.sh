...
# Find the Grim Reaper installation directory
# Try local installation first, then fallback to global
if [[ -d "./node_modules/grim-reaper" ]]; then
    GRIM_ROOT="./node_modules/grim-reaper"
elif [[ -d "/usr/lib/node_modules/grim-reaper" ]]; then
    GRIM_ROOT="/usr/lib/node_modules/grim-reaper"
elif [[ -d "/usr/local/lib/node_modules/grim-reaper" ]]; then
    GRIM_ROOT="/usr/local/lib/node_modules/grim-reaper"
elif [[ -d "$(npm root -g)/grim-reaper" ]]; then
    GRIM_ROOT="$(npm root -g)/grim-reaper"
else
    echo "❌ Grim Reaper installation not found. Please install with: npm install -g grim-reaper"
    exit 1
fi
cd "$GRIM_ROOT"
...
if [[ $# -eq 0 ]]; then
    ...
    echo "  setup-dependencies       Install required dependencies"
    echo ""
    echo "Examples:"
...
# Function to get Python command with virtual environment support
get_python_cmd() {
    # Check if Grim Reaper virtual environment exists
    if [[ -f "/opt/grim-reaper/venv/bin/python" ]]; then
        echo "/opt/grim-reaper/venv/bin/python"
...
# Run dependency check for critical commands
if [[ "$COMMAND" == "health" || "$COMMAND" == "backup" || "$COMMAND" == "status" ]]; then
    if [[ -f "scripts/dependency-manager.sh" ]]; then
        # Quick dependency check without installation prompts
        ...
        if [[ "$PYTHON_OK" == "false" || "$GO_OK" == "false" ]]; then
            echo -e "\033[0;31m❌ CRITICAL: Missing dependencies detected!\033[0m"
            echo ""
            echo "Grim Reaper requires the following dependencies:"
            [[ "$PYTHON_OK" == "false" ]] && echo "  ❌ Python 3 (required for py_grim and scythe)"
            [[ "$GO_OK" == "false" ]] && echo "  ❌ Go (required for go_grim compression)"
            echo ""
            echo "To fix this, run:"
            echo "  npm install -g grim-reaper@latest"
            echo "  grim setup-dependencies"
            echo ""
            echo "Or install manually:"
            [[ "$PYTHON_OK" == "false" ]] && echo "  - Python 3: https://www.python.org/downloads/"
            [[ "$GO_OK" == "false" ]] && echo "  - Go: https://golang.org/dl/"
            echo ""
            exit 1
        fi
    fi
...
case "$COMMAND" in
    # Core Operations
    health)
        $(get_python_cmd) scythe/scythe.py health
        ;;
    status)
        $(get_python_cmd) scythe/scythe.py status
        ;;
    backup)
        if [[ $# -eq 0 ]]; then
            error "Usage: grim backup <path> [--name <name>]"
        fi
        $(get_python_cmd) scythe/scythe.py backup "$@"
        ;;
    restore)
        if [[ $# -eq 0 ]]; then
    ...
    monitor)
        ...
        ;;
    web)
        $(get_python_cmd) py_grim/grim_web/app.py
        ;;
    ...
    setup-dependencies)
        if [[ -f "scripts/dependency-manager.sh" ]]; then
            ...
            error "Dependency manager script not found"
        fi
        ;;
