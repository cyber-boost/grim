...
client = GrimClient(api_key="YOUR_API_KEY")
...
EOF
    ...
    for md_file in "$DOCS_DIR"/*.md; do
        if [[ -f "$md_file" ]]; then
            ...
            cat "$DOCS_TEMPLATES/html_template.html" | sed "s|<!-- Content will be inserted here -->|$(cat "$md_file" | sed 's/^# /<h1>/; s/^## /<h2>/; s/^### /<h3>/; s/`\([^`]*\)`/<code>\1<\/code>/g; s/\*\*\([^*]*\)\*\*/<strong>\1<\/strong>/g; s/\*\([^*]*\)\*/<em>\1<\/em>/g')|" > "$html_file"
