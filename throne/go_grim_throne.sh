#!/bin/bash
# Grim Reaper Unified Command Router - Main Maestro
# Routes all grim commands to appropriate modules (Python, Shell, Go, etc.)
...
# ASCII Art Display - Using approved ASCII art
display_ascii() {
    cat "$GRIM_ROOT/ascii/ascii/grim-4.txt"
    echo ""
    echo "Grim Reaper - Unified Command Interface"
}
...
if [[ $# -eq 0 ]]; then
    ...
    echo "  compress-*               Compression operations"
    echo ""
    echo "Examples:"
    ...
    echo "  grim compress file.txt   # Compress file"
    echo ""
...
case "$COMMAND" in
    ...
    # Compression Operations (Go-powered)
    compress)
        if [[ $# -eq 0 ]]; then
    ...
    compress-benchmark)
        ...
        if [[ -f "go_grim/build/grim-compression" ]]; then
            ./go_grim/build/grim-compression -benchmark -input "$@"
        else
    ...
    decompress)
        ...
        if [[ -f "go_grim/build/grim-compression" ]]; then
            ./go_grim/build/grim-compression -decompress -input "$@"
        else
    ...
    go-build)
        ...
        go build -o build/grim-scanner ./cmd/scanner
        go build -o build/grim-transfer ./cmd/transfer
        success "Go binaries built successfully"
        ;;
    go-test)
        info "Testing Go components..."
        cd go_grim
        go test ./...
    ...
    # Help
    help-all)
        cat "$GRIM_ROOT/ascii/ascii/commands.txt" 2>/dev/null || echo "Full command list not available"
        ;;
