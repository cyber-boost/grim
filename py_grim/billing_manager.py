...
class BillingManager:
    ...
    def __init__(self):
        ...
        # Tier pricing and limits will be fetched from GRIMS_MOTHER API
        # Local cache for performance
        self._tier_cache = {}
    ...
    def _get_grims_mother_connection(self):
        """Get connection to GRIMS_MOTHER API"""
        if not self.grims_mother_url:
            raise ValueError("GRIMS_MOTHER environment variable not set")
        
        # GRIMS_MOTHER should be the API URL, not a database connection
        return self.grims_mother_url
    
    def _is_cache_valid(self) -> bool:
        """Check if tier cache is still valid"""
        if not self._tier_cache or not self._cache_expiry:
            return False
        return datetime.now() < self._cache_expiry
    
    def _fetch_tier_data_from_api(self) -> Dict[str, Any]:
        """Fetch tier definitions from GRIMS_MOTHER API"""
        try:
            import requests
            grims_mother_url = self._get_grims_mother_connection()
            api_url = f"{grims_mother_url}/grim/tiers"
            ...
            logger.error(f"Failed to fetch tier data from API: {e}")
        
        return {}
    
    def _get_tier_data(self) -> Dict[str, Any]:
        """Get tier data from cache or API"""
        if self._is_cache_valid():
            return self._tier_cache
        
        # Fetch from API
        tier_data = self._fetch_tier_data_from_api()
        if tier_data:
            self._tier_cache = tier_data
            self._cache_expiry = datetime.now() + timedelta(seconds=self._cache_duration)
            return tier_data
        
        # Fallback to default values if API fails
        logger.warning("Using fallback tier data due to API failure")
        return {
            'tier_prices': {
                'PRO': {'monthly': 2000, 'annual': 20000, 'currency': 'usd'},
                'MASTER': {'monthly': 4900, 'annual': 49000, 'currency': 'usd'},
                'REAPER': {'monthly': 9900, 'annual': 99000, 'currency': 'usd'}
            },
            'storage_limits': {
                'FREE': 5, 'PRO': 25, 'MASTER': 100, 'REAPER': 500
            }
        }
    
    def get_tier_prices(self) -> Dict[str, Any]:
        """Get tier pricing from GRIMS_MOTHER API"""
        tier_data = self._get_tier_data()
        return tier_data.get('tier_prices', {})
    
    def get_storage_limits(self) -> Dict[str, int]:
        """Get storage limits from GRIMS_MOTHER API"""
        tier_data = self._get_tier_data()
        return tier_data.get('storage_limits', {})
    
    def get_all_plans(self) -> Dict[str, Any]:
        """Get all available subscription plans from GRIMS_MOTHER API"""
        tier_prices = self.get_tier_prices()
        storage_limits = self.get_storage_limits()
            ...
            'plans': [
                {
                    ...
                    'currency': 'usd',
                    'billing_cycle': 'none',
                    'storage_gb': storage_limits.get('FREE', 5),
                    'features': [
                        'Basic auto-backup',
                        f"{storage_limits.get('FREE', 5)} GB storage",
                        'Email support'
                    ]
                },
                {
                    'id': 'PRO',
                    'name': 'Pro',
                    'price': tier_prices.get('PRO', {}).get('monthly', 2000) / 100,  # Convert cents to dollars
                    'currency': 'usd',
                    'billing_cycle': 'monthly',
                    'storage_gb': storage_limits.get('PRO', 25),
                    'features': [
                        'Advanced auto-backup',
                        f"{storage_limits.get('PRO', 25)} GB storage",
                        'Priority support',
                ...
                },
                {
                    'id': 'MASTER',
                    'name': 'Master',
                    'price': tier_prices.get('MASTER', {}).get('monthly', 4900) / 100,  # Convert cents to dollars
                    'currency': 'usd',
                    'billing_cycle': 'monthly',
                    'storage_gb': storage_limits.get('MASTER', 100),
                    'features': [
                        'Everything in Pro',
                        f"{storage_limits.get('MASTER', 100)} GB storage",
                        '24/7 support',
                        'Advanced encryption',
                ...
                },
                {
                    'id': 'REAPER',
                    'name': 'Reaper',
                    'price': tier_prices.get('REAPER', {}).get('monthly', 9900) / 100,  # Convert cents to dollars
                    'currency': 'usd',
                    'billing_cycle': 'monthly',
                    'storage_gb': storage_limits.get('REAPER', 500),
                    'features': [
                        'Everything in Master',
                        f"{storage_limits.get('REAPER', 500)} GB storage",
                        'Dedicated support',
                        'Enterprise features',
                        'Custom integrations'
                    ]
    ...
    def get_user_billing_info(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            # Try GRIMS_MOTHER API first
            grims_mother_url = self._get_grims_mother_connection()
            if grims_mother_url:
                try:
                    import requests
                    api_url = f"{grims_mother_url}/grim/user/{user_id}/billing"
                    response = requests.get(api_url, timeout=10)
                    
                    if response.status_code == 200:
                        user_data = response.json()
                        if user_data.get('success'):
                            billing_info = user_data.get('data', {})
                            ...
                            return {
                                'user_id': user_id,
                                'license_key': billing_info.get('license_key'),
                                'tier': billing_info.get('tier'),
                                'stripe_customer_id': billing_info.get('stripe_customer_id'),
                                'stripe_subscription_id': stripe_subscription_id,
                                'subscription_info': subscription_info,
                                'storage_limit_gb': self.get_storage_limits().get(billing_info.get('tier'), 5),
                                'source': 'grims_mother_api'
                            }
                except Exception as e:
                    logger.warning(f"Failed to get user info from GRIMS_MOTHER API: {e}")
            ...
            with sqlite3.connect(self.local_db_path) as conn:
                ...
                if row:
                    ...
                    return {
                        ...
                        'stripe_subscription_id': stripe_subscription_id,
                        'status': status,
                        'storage_limit_gb': self.get_storage_limits().get(tier, 5),
                        'source': 'local_cache'
                    }
    ...
    def create_payment_intent(self, user_id: str, plan_name: str, billing_cycle: str = 'monthly', 
        ...
        try:
            ...
            # Get tier pricing from API
            tier_prices = self.get_tier_prices()
            
            # Validate plan
            if plan_name not in tier_prices:
                return {'success': False, 'error': 'Invalid plan'}
            
            plan_config = tier_prices[plan_name]
            amount = plan_config.get(billing_cycle, plan_config.get('monthly', 2000))
    ...
    def create_subscription(self, user_id: str, plan_name: str, billing_cycle: str = 'monthly',
        ...
        try:
            ...
            # Create subscription with amount-based pricing
            subscription_data = {
                'customer': stripe_customer_id,
                'items': [{
                    'price_data': {
