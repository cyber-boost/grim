...
install_grim() {
    ...
    cp -r "$build_dir"/* "$GRIM_INSTALL_DIR/"
    
    # Set permissions (user ownership, not root)
    chown -R $USER:$USER "$GRIM_INSTALL_DIR"
    chmod -R 755 "$GRIM_INSTALL_DIR"
    find "$GRIM_INSTALL_DIR" -name "*.sh" -exec chmod +x {} \;
    find "$GRIM_INSTALL_DIR" -name "*.py" -exec chmod +x {} \;
    
    # grim_throne.sh now auto-detects its path - no manual updates needed
    
    # Install grim command (requires sudo for /usr/local/bin)
...
setup_environment() {
    ...
    mkdir -p "$GRIM_INSTALL_DIR"/{logs,db,backups,tmp,builds}
    mkdir -p "$GRIM_GRAVEYARD"
    
    # Set ownership to current user (not root)
    chown -R $USER:$USER "$GRIM_INSTALL_DIR"
    chown -R $USER:$USER "$GRIM_GRAVEYARD"
    
    # Set up PATH in bashrc
    if ! grep -q "$GRIM_INSTALL_DIR" "$HOME/.bashrc" 2>/dev/null; then
        echo "export PATH=\"$GRIM_INSTALL_DIR/sh_grim:\$PATH\"" >> "$HOME/.bashrc"
    fi
    ...
    if command -v python3 >/dev/null 2>&1; then
        ...
        # First ensure python3-venv is installed BEFORE any venv checks
        log "Ensuring python3-venv is installed..."
        venv_package_installed=false
        
        if command -v apt >/dev/null 2>&1; then
            # Get the Python version for the correct venv package
            PYTHON_VERSION=$(python3 -c "import sys; print(f'{sys.version_info.major}.{sys.version_info.minor}')")
            log "Python version detected: $PYTHON_VERSION"
            
            # Install the specific python venv package - this is critical for Ubuntu 24.04
            log "Installing python${PYTHON_VERSION}-venv package..."
            if [[ $EUID -ne 0 ]]; then
                # First update package list
                sudo apt update -qq
                # Install the specific version venv package first
                sudo apt install -y python${PYTHON_VERSION}-venv
                # Then install general packages
                sudo apt install -y python3-venv python3-pip python3-dev python3-full
            else
                # First update package list
                apt update -qq
                # Install the specific version venv package first
                ...
                # Then install general packages
                apt install -y python3-venv python3-pip python3-dev python3-full
            fi
            if [[ $? -eq 0 ]]; then
                venv_package_installed=true
                success "python3-venv packages installed"
            else
                warning "Failed to install python3-venv via apt"
            fi
        elif command -v yum >/dev/null 2>&1; then
            if [[ $EUID -ne 0 ]]; then
            ...
            if [[ $EUID -ne 0 ]]; then
                sudo dnf install -y python3-venv python3-pip python3-devel
            else
        ...
        # Only proceed with venv creation if packages were installed
        if [[ "$venv_package_installed" != "true" ]]; then
            warning "Could not install python3-venv packages - Python dependencies will use system Python"
        fi
        
        # Now check if python3-venv is available after installation
        if ! python3 -c "import venv" 2>/dev/null; then
            log "venv module still not available after package installation"
            warning "Will proceed without virtual environment"
        fi
        ...
        # Only try to create venv if the module is available
        if python3 -c "import venv" 2>/dev/null; then
            log "Using python3 -m venv..."
            # Remove any existing failed venv directory first
            rm -rf grim_venv
            ...
            if python3 -m venv grim_venv; then
                ...
                # Try without pip to see if that works
                if python3 -m venv --without-pip grim_venv; then
                    venv_created=true
                    warning "Virtual environment created without pip - will install pip separately"
                fi
            fi
        fi
...
EOF
        ...
        # Update grim_throne.sh to use virtual environment if it has hardcoded paths
        if [[ -f "$GRIM_INSTALL_DIR/grim_throne.sh" ]]; then
            sed -i "s|/opt/reaper|$GRIM_INSTALL_DIR|g" "$GRIM_INSTALL_DIR/grim_throne.sh"
            sed -i "s|/opt/grim_venv|$GRIM_INSTALL_DIR/grim_venv|g" "$GRIM_INSTALL_DIR/grim_throne.sh"
        fi
...
EOF
        ...
        # Substitute the actual install directory in the grim command
        sed -i "s|\$GRIM_INSTALL_DIR|$GRIM_INSTALL_DIR|g" /tmp/grim_new
        
        # Install the new grim command
        if [[ -f "/usr/local/bin/grim" ]]; then
            $SUDO cp /usr/local/bin/grim /usr/local/bin/grim.backup
        fi
        $SUDO cp /tmp/grim_new /usr/local/bin/grim
        $SUDO chmod +x /usr/local/bin/grim
...
show_help() {
    ...
    echo "  --install-dir <dir> Install directory (default: ~/.graveyard/reaper)"
