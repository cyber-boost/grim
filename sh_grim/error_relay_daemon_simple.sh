#!/bin/bash
# Simplified Automatic Error Relay Daemon for Grim Reaper System
...
# API Configuration
API_ENDPOINT="${GRIM_API_ENDPOINT:-http://localhost:4746}"
API_KEY="${GRIM_API_KEY:-default-api-key}"
APP_VERSION="1.0.0"
...
call_api() {
    ...
    if ! command_exists curl; then
        log_error "curl not available - cannot connect to central API"
        return 1
    fi
    ...
    if [[ $? -ne 0 ]]; then
        log_warning "Failed to connect to central API at $api_url"
        return 1
    fi
    ...
    if [[ "$http_code" -ge 200 && "$http_code" -lt 300 ]]; then
        ...
        log_warning "API call failed with HTTP $http_code: $response_body"
        return 1
    fi
}
...
# ============================================================================
show_help() {
    echo "Grim Reaper Error Relay Daemon (Simplified)"
    echo ""
