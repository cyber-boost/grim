...
show_help() {
    ...
    echo "  recover <source> <target> [options]  - Restore backup to target location (primary command)"
    echo "  restore <source> <target> [options]  - Restore backup to target location (alias for recover)"
    echo "  list [type]                          - List available backups"
    echo "  list-auto                            - List auto-backup files"
    echo "  restore-auto <file> <target>         - Restore from auto-backup"
    ...
    echo "  grim restore recover /backups/daily/backup.tar.gz /target/dir"
...
list_auto_backups() {
    echo -e "${CYAN}=== Auto-Backup Files ===${NC}"
    echo ""
    
    local auto_backup_dir="/root/.graveyard/auto_backups"
    local encrypted_dir="/root/.graveyard/auto_backups_encrypted"
    
    # Check license tier for access
    local license_key="${GRIM_LICENSE_KEY:-FREE}"
    local has_premium=false
    
    if [ "$license_key" != "FREE" ] && [ -n "$license_key" ]; then
        has_premium=true
    fi
    
    # List unencrypted backups (for premium users)
    if [ "$has_premium" = true ] && [ -d "$auto_backup_dir" ]; then
        echo -e "${GREEN}📁 Unencrypted Auto-Backups (Premium):${NC}"
        find "$auto_backup_dir" -type f -name "*.zstd" -o -name "*.gz" -o -name "*.xz" 2>/dev/null | sort | while read -r file; do
            local size=$(stat -c%s "$file" 2>/dev/null || echo "0")
            local date=$(stat -c%y "$file" 2>/dev/null | cut -d' ' -f1,2 | cut -d'.' -f1)
            local rel_path="${file#$auto_backup_dir/}"
    ...
    # List encrypted backups (for all users)
    if [ -d "$encrypted_dir" ]; then
        echo -e "${YELLOW}🔒 Encrypted Auto-Backups (FREE License):${NC}"
        find "$encrypted_dir" -type f -name "*.enc" 2>/dev/null | sort | while read -r file; do
            local size=$(stat -c%s "$file" 2>/dev/null || echo "0")
            local date=$(stat -c%y "$file" 2>/dev/null | cut -d' ' -f1,2 | cut -d'.' -f1)
    ...
    echo "  grim restore restore-auto <backup_file> <target_location>"
...
restore_auto_backup() {
    ...
    if [ -z "$backup_file" ]; then
        ...
        echo "Usage: grim restore restore-auto <backup_file> <target_location>"
        echo ""
        echo "List available files with: grim restore list-auto"
        return 1
    fi
    ...
    if [[ "$backup_file" == *.enc ]]; then
        echo -e "${YELLOW}🔒 Encrypted backup detected${NC}"
        ...
        if command -v openssl >/dev/null 2>&1; then
            if openssl enc -aes-256-cbc -d -in "$backup_file" -out "$decrypted_file" -k "$encryption_key" 2>/dev/null; then
                echo "✅ Decryption successful"
                backup_file="$decrypted_file"
            else
                echo -e "${RED}❌ Decryption failed - wrong key or corrupted file${NC}"
                echo "Try: grim auto-backup decrypt \"$backup_file\""
                return 1
            fi
        else
            echo -e "${RED}❌ OpenSSL not available for decryption${NC}"
            return 1
        fi
    fi
    
    # Restore the backup
    echo -e "${CYAN}📦 Restoring auto-backup...${NC}"
    ...
    # Try Go compression engine first
    local go_compression="$GRIM_ROOT/go_grim/build/grim-compression"
    if [ -x "$go_compression" ]; then
        if "$go_compression" -decompress -input "$backup_file" -output "$target_location" >/dev/null 2>&1; then
            echo -e "${GREEN}✅ Restored using Go compression engine${NC}"
            restored=true
        fi
    fi
        ...
        case "$backup_file" in
            *.zstd)
                if command -v zstd >/dev/null 2>&1; then
                    zstd -d "$backup_file" -o "$target_location/$(basename "${backup_file%.zstd}")"
                    restored=true
                fi
                ;;
            *.gz)
                gunzip -c "$backup_file" > "$target_location/$(basename "${backup_file%.gz}")"
                restored=true
                ;;
            *.xz)
                if command -v xz >/dev/null 2>&1; then
                    xz -d -c "$backup_file" > "$target_location/$(basename "${backup_file%.xz}")"
                    restored=true
                fi
                ;;
        esac
    ...
    if [ "$restored" = true ]; then
        echo -e "${GREEN}🎉 Auto-backup restore completed successfully!${NC}"
        echo "📁 Files restored to: $target_location"
        
        # Clean up decrypted temp file if it was created
        if [[ "$1" == *.enc ]] && [ -f "$decrypted_file" ]; then
            rm -f "$decrypted_file"
            echo "🧹 Temporary decrypted file cleaned up"
        fi
        
        log "Auto-backup restored: $1 -> $target_location"
    else
        echo -e "${RED}❌ Failed to restore backup${NC}"
        echo "Unsupported backup format or missing tools"
        return 1
    fi
}
