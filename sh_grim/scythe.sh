...
# ============================================================================
# DYNAMIC PATH DETECTION AND GRIM ROOT SETUP
# ============================================================================
...
# Dynamic GRIM_ROOT detection with fallback chain
get_grim_root() {
    # Priority 1: Check if we're in sh_grim directory, go up one level (script location takes precedence)
    local script_dir="$(get_script_dir)"
    local parent_dir="$(dirname "$script_dir")"
    
    # Check if parent looks like grim root (has key directories)
    if [[ -d "$parent_dir/sh_grim" && -d "$parent_dir/config" ]]; then
        echo "$parent_dir"
        return 0
    fi
    
    # Priority 2: Environment variable
    if [[ -n "${GRIM_ROOT:-}" && -d "$GRIM_ROOT" ]]; then
        echo "$GRIM_ROOT"
        return 0
    fi
    
    # Priority 3: Look for .graveyard in current and parent directories
    local current="$(pwd)"
    while [[ "$current" != "/" ]]; do
        if [[ -d "$current/.graveyard" ]]; then
            echo "$current"
            return 0
        fi
    ...
    # Priority 5: Default to parent of script directory
    echo "$parent_dir"
}

# Get or create .graveyard/.rip/.scythe structure
setup_scythe_paths() {
    local grim_root="$1"
    
    # Create .graveyard/.rip/.scythe structure
    local graveyard_dir="$grim_root/.graveyard"
    local rip_dir="$graveyard_dir/.rip"
    local scythe_dir="$rip_dir/.scythe"
    
    # Ensure directories exist
    mkdir -p "$scythe_dir" 2>/dev/null || true
    
    # Set proper permissions
    chmod 750 "$graveyard_dir" "$rip_dir" "$scythe_dir" 2>/dev/null || true
    
    echo "$scythe_dir"
}

# Initialize GRIM environment
SCRIPT_DIR="$(get_script_dir)"
GRIM_ROOT="$(get_grim_root)"
SCYTHE_HOME="$(setup_scythe_paths "$GRIM_ROOT")"

# ============================================================================
# COLORS AND UTILITIES (Self-contained, no external dependencies)
# ============================================================================
...
# Command existence check (self-contained)
command_exists() {
    command -v "$1" >/dev/null 2>&1
}

# Logging function with security considerations
log_action() {
    local level="${1:-INFO}"
    local message="$2"
    ...
    echo "[$timestamp] [$level] $message" | tee -a "$SCYTHE_LOG"
}

# Security: Validate script is running with proper permissions
if [[ ! -r "$SCRIPT_DIR" ]]; then
    echo "ERROR: Cannot read script directory" >&2
    exit 1
fi

# ============================================================================
# SCYTHE CONFIGURATION WITH DYNAMIC PATHS
# ============================================================================

SCYTHE_VERSION="2.0.0"

# New .graveyard/.rip/.scythe structure
SCYTHE_CONFIG="$SCYTHE_HOME/scythe.tsk"
SCYTHE_DB="$SCYTHE_HOME/scythe.db"
SCYTHE_LOG="$GRIM_ROOT/logs/scythe.log"
SCYTHE_PID="$SCYTHE_HOME/scythe.pid"

# Updated Mother DB URL (changed from api.grim.so to rip.grim.so)
MOTHER_DB_URL="${MOTHER_DB_URL:-https://rip.grim.so/scythe}"
SCYTHE_CHECK_INTERVAL="${SCYTHE_CHECK_INTERVAL:-3600}" # 1 hour default

# Ensure required directories exist
mkdir -p "$(dirname "$SCYTHE_LOG")" 2>/dev/null || true
mkdir -p "$SCYTHE_HOME" 2>/dev/null || true

# Security: Validate scythe environment (self-contained validation)
validate_environment() {
    local issues=()
    
    # Check if GRIM_ROOT is accessible
    if [[ ! -d "$GRIM_ROOT" ]]; then
        issues+=("GRIM_ROOT directory not accessible: $GRIM_ROOT")
    fi
    
    # Check if scythe home is accessible
    if [[ ! -d "$SCYTHE_HOME" ]]; then
        if ! mkdir -p "$SCYTHE_HOME" 2>/dev/null; then
            issues+=("Cannot create SCYTHE_HOME: $SCYTHE_HOME")
        fi
    fi
    
    # Check if logs directory is accessible
    local log_dir="$(dirname "$SCYTHE_LOG")"
    if [[ ! -d "$log_dir" ]]; then
        if ! mkdir -p "$log_dir" 2>/dev/null; then
            issues+=("Cannot create log directory: $log_dir")
        fi
    fi
    ...
    # Export environment for compatibility
    export GRIM_ROOT
    export SCYTHE_HOME
    export SCYTHE_CONFIG
    export SCYTHE_DB
    export SCYTHE_LOG
    export SCYTHE_PID
...
validate_path() {
    ...
    local resolved_path=$(readlink -f "$path" 2>/dev/null || echo "$path")
    local resolved_base=$(readlink -f "$base_dir" 2>/dev/null || echo "$base_dir")
...
# Enhanced .tsk configuration parser with security validation
parse_tusk_config() {
    local config_file="$1"
    local section="$2"
    local key="$3"
    ...
    if ! validate_path "$config_file" "$GRIM_CONFIG_DIR"; then
            ...
            if [[ "$line" =~ ^[[:space:]]*([a-zA-Z_][a-zA-Z0-9_]*):[[:space:]]*(.+)$ ]]; then
                ...
                if [[ "$current_key" == "$key" ]]; then
                    ...
                    if [[ ${#value} -gt 1024 ]]; then
                        log_error "Config value too long for key: $key"
                        return 1
                    fi
...
EOF
    ...
    if declare -f create_integration_script >/dev/null; then
        create_integration_script "$target_dir" "$software_id"
    else
        echo "Warning: create_integration_script function not found"
    fi
...
Configuration:
  GRIM_ROOT: $GRIM_ROOT
  Scythe Home: $SCYTHE_HOME
  Main config: $SCYTHE_CONFIG
  Database: $SCYTHE_DB
  Log file: $SCYTHE_LOG
  Mother DB URL: $MOTHER_DB_URL
...
class Scythe {
    ...
    validateLicense() {
        ...
        const options = {
            hostname: 'rip.grim.so',
            path: '/scythe/validate',
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Content-Length': data.length,
                'X-Scythe-Version': '2.0'
            }
        };
...
class Scythe:
    ...
    def _report_violation(self, violation_type):
        try:
            requests.post(
                'https://rip.grim.so/scythe/violation',
                json={
                    'software_id': self.config['software']['id'],
...
impl Scythe {
    ...
    fn validate_license(config: &Config) {
        ...
        let response = client
            .post("https://rip.grim.so/scythe/validate")
            .json(&payload)
            .send();
    ...
    fn report_violation(config: &Config, violation_type: &str) {
        ...
        let client = reqwest::blocking::Client::new();
        let _ = client
            .post("https://rip.grim.so/scythe/violation")
            .json(&payload)
            .send();
    }
}
...
class Scythe
  ...
  def validate_license
    ...
    uri = URI('https://rip.grim.so/scythe/validate')
  ...
  def report_violation(violation_type)
    ...
    uri = URI('https://rip.grim.so/scythe/violation')
    http = Net::HTTP.new(uri.host, uri.port)
    http.use_ssl = true
...
public class Scythe {
    ...
    private void validateLicense() {
        try {
            ...
            URL url = new URL("https://rip.grim.so/scythe/validate");
    ...
    private void reportViolation(String violationType) {
        try {
            ...
            URL url = new URL("https://rip.grim.so/scythe/violation");
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
            conn.setRequestMethod("POST");
            conn.setRequestProperty("Content-Type", "application/json");
            conn.setDoOutput(true);
...
check_scythe_license() {
    ...
    # Validate with mother DB (updated URL)
    local response=\$(curl -s -X POST "https://rip.grim.so/scythe/validate" \\
        -H "Content-Type: application/json" \\
        -H "X-Scythe-Version: 2.0" \\
...
report_scythe_violation() {
    ...
    curl -s -X POST "https://rip.grim.so/scythe/violation" \\
        -H "Content-Type: application/json" \\
        -H "X-Scythe-Version: 2.0" \\
