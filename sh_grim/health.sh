...
check_modules() {
    ...
    local module_locations=(
        "$GRIM_ROOT/modules"
        "$(dirname "$SCRIPT_PATH")/../modules"
        "/root/.graveyard/reaper/modules"
        "/opt/reaper/modules"
        "$HOME/.graveyard/reaper/modules"
    )
    
    local modules_dir=""
    for location in "${module_locations[@]}"; do
        if [ -d "$location" ]; then
            modules_dir="$location"
            log "Found modules directory: $location"
            break
        fi
    done
    
    if [ -z "$modules_dir" ]; then
        echo -e "${RED}❌ Modules directory: Not found in any expected location${NC}"
        record_health_check "modules_directory" "critical" "Modules directory not found" "Modules directory missing from all expected locations" "critical"
        return 1
    fi
    ...
    for module in "${core_modules[@]}"; do
        if [ -f "$modules_dir/$module" ] && [ -x "$modules_dir/$module" ]; then
            ...
            record_health_check "module_$module" "ok" "Module $module accessible" "Module file exists and executable at $modules_dir" "info"
        else
            echo -e "${RED}❌ Module $module: Missing or not executable${NC}"
            record_health_check "module_$module" "critical" "Module $module missing" "Module file missing or not executable from $modules_dir" "critical"
            issues_found=$((issues_found + 1))
        fi
    done
    ...
    for module in "${new_modules[@]}"; do
        if [ -f "$modules_dir/$module" ] && [ -x "$modules_dir/$module" ]; then
            ...
            record_health_check "module_$module" "warning" "Module $module missing" "Module file missing or not executable from $modules_dir" "warning"
            issues_found=$((issues_found + 1))
        fi
    done
