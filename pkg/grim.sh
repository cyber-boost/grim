...
update_package_version() {
    ...
    if [[ $(echo "$pkg_info" | jq -r '.files.json_field // empty') ]]; then
        ...
        # Pattern replacement - improved logic
        local file=$(echo "$pkg_info" | jq -r '.files.version')
        local pattern=$(echo "$pkg_info" | jq -r '.files.pattern')
        
        if [[ -f "$file" ]]; then
            # Create a more robust search pattern with proper Perl regex
            local search_pattern=$(echo "$pattern" | sed 's/VERSION/[0-9]+\.[0-9]+\.[0-9]+/g')
            local replace_pattern=$(echo "$pattern" | sed "s/VERSION/$VERSION/g")
            
            # Use perl for more reliable pattern matching
            perl -pi -e "s|$search_pattern|$replace_pattern|g" "$file"
            log "✅ Updated $package version to $VERSION" "$GREEN"
        fi
    fi
}
...
build_javascript() {
    ...
    log "⚠️  Skipping tests to avoid hanging..." "$YELLOW"
    npm pack
...
build_python() {
    ...
    pip install build twine || log "⚠️  Build tools installation failed" "$YELLOW"
    
    # Use modern python build instead of deprecated setup.py
    python -m build
...
build_php() {
    ...
    composer update --no-dev || log "⚠️  Some dependencies failed" "$YELLOW"
